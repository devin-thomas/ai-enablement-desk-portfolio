...
function RequestDetailView({ request, analyses, clarificationAnswers, decisions, automations, artifacts, analysisBusy, audioBusy, focusHeading, onBack, onAnalyze, onAnswer, onDecision, onAutomationRetry, onAudio }: { request: RequestDetail; analyses: AnalysisRun[]; clarificationAnswers: ClarificationAnswerRecord[]; decisions: DecisionRecord[]; automations: AutomationAttempt[]; artifacts: ArtifactRecord[]; analysisBusy: boolean; audioBusy: boolean; focusHeading: boolean; onBack: () => void; onAnalyze: () => Promise<void>; onAnswer: (submission: ClarificationAnswerSubmission) => Promise<void>; onDecision: (submission: HumanDecision) => Promise<void>; onAutomationRetry: (attemptId: string) => Promise<void>; onAudio: () => Promise<void> }) {
  ...
  return <>
    ...
    <div className="detail-header"><div><div className={`request-type-banner ${request.requestType}`}><span>Category</span><strong>{requestTypeLabel(request.requestType)}</strong></div><div className="detail-id">Request {displayRequestId(request.id)}</div><h2 ref={headingRef} tabIndex={-1}>{request.title}</h2><p>{request.requesterName} · {request.department}</p></div><button className="secondary-button" disabled={!canAnalyze || analysisBusy} onClick={() => void onAnalyze()}><Sparkles size={14} />{analysisBusy ? 'Reviewing…' : analyses.length ? 'Update recommendation' : 'Review request'}</button></div>
    ...
    <div className="audio-summary"><div><Headphones size={16} aria-hidden="true" /><strong>Source briefing</strong></div>{artifacts.length ? artifacts.map((artifact) => <audio key={artifact.id} controls preload="none" src={artifact.contentUrl} aria-label={`Play source briefing for ${request.title}`}>Audio could not be played.</audio>) : <span>{isRecruiterDemo ? 'Audio unavailable in this public demo.' : 'No audio attached.'}</span>}</div>
    ...
    <div className="original-request"><span>Original request</span><p>{request.requestText ?? request.businessProblem}</p></div>
