...
import { answerClarification, generateAudioBriefing, getHealth, getRequest, listAnalyses, listArtifacts, listAutomations, listDecisions, listRequests, recordDecision, resetDemo, retryAutomation, runAnalysis, submitRequest } from './api'
...
function App() {
  ...
  const [geminiStatus, setGeminiStatus] = useState<'configured' | 'unavailable_key' | 'unavailable'>('unavailable')
  const [n8nStatus, setN8nStatus] = useState<'configured' | 'disabled' | 'unavailable_secret' | 'unavailable'>('unavailable')
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      <header className="topbar"><div className="breadcrumbs"><span>Operations</span><span>/</span><strong>Intake queue</strong></div><div className="system-status"><span className="pulse" />Persisted request service</div></header>
      <div className="page-heading"><div><p className="eyebrow">Restart-safe workflow</p><div className="portfolio-label"><span className="portfolio-dot" />Synthetic data only</div><h1>Intake queue</h1><p className="lede">Submitted records and audit events are loaded from Postgres.</p></div><div className="heading-actions"><button className="secondary-button scenario-button" disabled={resetting} onClick={() => void handleReset()}><RefreshCw size={15} />{resetting ? 'Resetting…' : 'Reset demo'}</button><button className="primary-button" onClick={() => setShowNewRequest(true)}><Plus size={17} />New request</button></div></div>
      <section className="queue-strip"><div className="queue-stat"><span>Persisted requests</span><strong>{requests.length}</strong><small>Current database</small></div><div className="queue-stat"><span>AI projects</span><strong>{requests.filter((r) => r.requestType === 'ai_project').length}</strong><small>Synthetic or submitted</small></div><div className="queue-stat"><span>Tool access</span><strong>{requests.filter((r) => r.requestType === 'tool_access').length}</strong><small>Synthetic or submitted</small></div><div className="queue-note"><Activity size={18} /><span><strong>Providers:</strong> Gemini {geminiStatus}; n8n {n8nStatus}; Fish Audio {fishStatus}.</span></div></section>
      ...
      <section className="governance-note"><LockKeyhole size={18} /><div><strong>Persistence boundary</strong><span>The browser holds only a view cache. Validation, durable IDs, records, and initial audit events are owned by the server and database.</span></div></section>
    </main>
...
function RequestDetailView({ request, analyses, clarificationAnswers, decisions, automations, artifacts, analysisBusy, audioBusy, onAnalyze, onAnswer, onDecision, onAutomationRetry, onAudio }: { request: RequestDetail; analyses: AnalysisRun[]; clarificationAnswers: ClarificationAnswerRecord[]; decisions: DecisionRecord[]; automations: AutomationAttempt[]; artifacts: ArtifactRecord[]; analysisBusy: boolean; audioBusy: boolean; onAnalyze: () => Promise<void>; onAnswer: (submission: ClarificationAnswerSubmission) => Promise<void>; onDecision: (submission: HumanDecision) => Promise<void>; onAutomationRetry: (attemptId: string) => Promise<void>; onAudio: () => Promise<void> }) {
  ...
  return <>
    ...
    {latest ? <AnalysisView run={latest} request={request} clarificationAnswers={clarificationAnswers} onAnswer={onAnswer} /> : <div className="analysis-empty"><Sparkles size={18} /><div><strong>No analysis history</strong><span>Run advisory analysis when the server-side provider is configured.</span></div></div>}
...
function AutomationEvidence({ request, analyses, automations, artifacts, audioBusy, onRetry, onAudio }: { request: RequestDetail; analyses: AnalysisRun[]; automations: AutomationAttempt[]; artifacts: ArtifactRecord[]; audioBusy: boolean; onRetry: (attemptId: string) => Promise<void>; onAudio: () => Promise<void> }) {
  ...
  return <section className="automation-evidence">
    ...
    <p className="analysis-summary">The written analysis is authoritative. Audio is generated only for explicitly synthetic demo-safe requests and only after the provider returns real bytes.</p>
    {latestSummary && <div className="authoritative-text"><strong>Authoritative briefing text</strong><p>{latestSummary}</p><button className="play-button" disabled={!request.syntheticDemoSafe || audioBusy} onClick={() => void onAudio()}><Headphones size={14} />{audioBusy ? 'Generating…' : 'Generate optional audio'}</button></div>}
...
function ReviewerWorkspace({ request, analyses, decisions, onDecision }: { request: RequestDetail; analyses: AnalysisRun[]; decisions: DecisionRecord[]; onDecision: (submission: HumanDecision) => Promise<void> }) {
  ...
  return <section className="review-workspace">
    ...
    <p className="analysis-summary">A decision is bound to the latest successful analysis and this request version. The server rechecks legal transitions and governance eligibility.</p>
...
function RequestForm({ onClose, onSubmitted }: { onClose: () => void; onSubmitted: (request: RequestDetail) => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><form className="new-request-modal intake-form" role="dialog" aria-modal="true" aria-labelledby="request-form-title" onSubmit={(event) => void handleSubmit(event)} noValidate><button type="button" className="modal-close" onClick={onClose} aria-label="Close intake form"><X size={18} /></button><p className="eyebrow">Persisted intake</p><h2 id="request-form-title">Submit a request</h2><p>All fields are synthetic demo information. The server validates the same shared contract before writing anything.</p>
    ...
    {serverError && <p className="form-error" role="alert">{serverError}</p>}<button className="secondary-button full" disabled={submitting}>{submitting ? 'Submitting…' : 'Submit durable request'}</button>
  </form></div>
}
