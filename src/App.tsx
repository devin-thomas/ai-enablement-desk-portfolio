...
import { Activity, ArrowUpRight, FileClock, Headphones, LockKeyhole, Plus, RefreshCw, Search, Sparkles, Workflow, X } from 'lucide-react'
...
import { answerClarification, generateAudioBriefing, generateOriginalRequestNarration, getHealth, getRequest, isRecruiterDemo, listAnalyses, listArtifacts, listAutomations, listDecisions, listRequests, recordDecision, resetDemo, retryAutomation, runAnalysis, submitRequest } from './api'

const blankForm: RequestSubmission = {
  title: '', requestType: 'ai_project', department: '', requesterName: '', requesterRole: '',
  requestText: '', businessProblem: '', desiredOutcome: '', currentProcess: '', intendedUsers: [], dataSources: [],
...
function App() {
  ...
  const [geminiStatus, setGeminiStatus] = useState<'configured' | 'unavailable_key' | 'approval_required' | 'demo_fixture' | 'unavailable'>('unavailable')
  const [n8nStatus, setN8nStatus] = useState<'configured' | 'disabled' | 'unavailable_secret' | 'demo_evidence' | 'unavailable'>('unavailable')
  const [fishStatus, setFishStatus] = useState<'configured' | 'disabled' | 'unavailable_key' | 'approval_required' | 'unavailable'>('unavailable')
  const [automations, setAutomations] = useState<AutomationAttempt[]>([])
  const [artifacts, setArtifacts] = useState<ArtifactRecord[]>([])
  const [audioBusy, setAudioBusy] = useState(false)
  const [originalNarrationBusy, setOriginalNarrationBusy] = useState(false)
  ...
  async function handleOriginalRequestNarration() {
    if (!selected) return
    setOriginalNarrationBusy(true); setError(null)
    try { await generateOriginalRequestNarration(selected.id); await loadQueue(selected.id) }
    catch (audioError) { setError(audioError instanceof Error ? audioError.message : 'Original request narration is unavailable; the written request remains authoritative.') }
    finally { setOriginalNarrationBusy(false) }
  }

  return <div className={`app-shell ${mobileView === 'detail' ? 'mobile-detail-mode' : 'mobile-queue-mode'}`}>
    <main className="main-content">
      ...
      <div className={`content-grid ${mobileView === 'detail' ? 'mobile-detail-active' : ''}`}>
        <section className="request-list-panel mobile-queue-view"><div className="panel-heading"><div><h2>Requests</h2><span>{filteredRequests.length} visible</span></div><label className="search-box"><Search size={15} /><input aria-label="Search requests" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search requests" /></label></div>
          ...
          {loading ? <PanelState title="Loading requests…" /> : filteredRequests.length === 0 ? <PanelState title={requests.length === 0 ? 'No requests yet' : 'No matching requests'} /> : <div className="request-list" role="list">{filteredRequests.map((request) => <button className={request.id === selectedId ? 'request-row selected' : 'request-row'} key={request.id} onClick={() => void selectRequest(request.id)} aria-label={`${request.title}, ${requestTypeLabel(request.requestType)}, ${statusLabel(request.status)}, open request`}><div className="row-top"><span className={`request-type-label ${request.requestType}`}>{requestTypeLabel(request.requestType)}</span><span className="status compact blue"><span />{statusLabel(request.status)}</span></div><strong>{request.title}</strong><span className="row-meta">{request.department} · {new Date(request.submittedAt).toLocaleDateString()}</span><span className="row-open">Open request <ArrowUpRight size={13} aria-hidden="true" /></span></button>)}</div>}
        </section>
        <section className="detail-panel mobile-detail-view">{selected ? <RequestDetailView request={selected} analyses={analyses} clarificationAnswers={clarificationAnswers} decisions={decisions} automations={automations} artifacts={artifacts} analysisBusy={analysisBusy} audioBusy={audioBusy} originalNarrationBusy={originalNarrationBusy} focusHeading={mobileView === 'detail'} onBack={() => setMobileView('queue')} onAnalyze={handleAnalysis} onAnswer={handleClarification} onDecision={handleDecision} onAutomationRetry={handleAutomationRetry} onAudio={handleAudio} onOriginalRequestNarration={handleOriginalRequestNarration} /> : <PanelState title={loading ? 'Loading request…' : 'Select or submit a request'} />}</section>
      </div>
      <section className="governance-note"><LockKeyhole size={18} /><div><strong>{isRecruiterDemo ? 'Demo boundary' : 'Persistence boundary'}</strong><span>{isRecruiterDemo ? 'This public version uses local browser storage and validated example fixtures. The repository documents and tests the separate trusted server, Postgres, Gemini, n8n, and Fish boundaries.' : 'The browser holds only a view cache. Validation, durable IDs, records, and initial audit events are owned by the server and database.'}</span></div></section>
    </main>
...
function RequestDetailView({ request, analyses, clarificationAnswers, decisions, automations, artifacts, analysisBusy, audioBusy, originalNarrationBusy, focusHeading, onBack, onAnalyze, onAnswer, onDecision, onAutomationRetry, onAudio, onOriginalRequestNarration }: { request: RequestDetail; analyses: AnalysisRun[]; clarificationAnswers: ClarificationAnswerRecord[]; decisions: DecisionRecord[]; automations: AutomationAttempt[]; artifacts: ArtifactRecord[]; analysisBusy: boolean; audioBusy: boolean; originalNarrationBusy: boolean; focusHeading: boolean; onBack: () => void; onAnalyze: () => Promise<void>; onAnswer: (submission: ClarificationAnswerSubmission) => Promise<void>; onDecision: (submission: HumanDecision) => Promise<void>; onAutomationRetry: (attemptId: string) => Promise<void>; onAudio: () => Promise<void>; onOriginalRequestNarration: () => Promise<void> }) {
  const latest = analyses[0]
  const sourceBriefings = artifacts.filter((artifact) => artifact.artifactType === 'audio_briefing')
  const originalRequestNarrations = artifacts.filter((artifact) => artifact.artifactType === 'original_request_narration')
  ...
  const needsClarification = Boolean(latest?.modelAnalysis?.clarificationQuestions.some((question) => question.blocking))
  const nextAction = needsClarification ? 'Answer required question' : !latest ? 'Review request' : request.status === 'ready_for_review' ? 'Make a reviewer decision' : 'Review recommendation'
  return <>
    <div className="mobile-detail-nav"><button className="secondary-button" onClick={onBack}><span aria-hidden="true">←</span> Back to queue</button><span>Request {displayRequestId(request.id)}</span></div>
    <div className="detail-header"><div><div className={`request-type-banner ${request.requestType}`}><span>Category</span><strong>{requestTypeLabel(request.requestType)}</strong></div><div className="detail-id">Request {displayRequestId(request.id)}</div><h2 ref={headingRef} tabIndex={-1}>{request.title}</h2><p>{request.requesterName} · {request.department}</p></div><div className="detail-header-actions">{!isRecruiterDemo && <button className="secondary-button" disabled={!request.syntheticDemoSafe || originalNarrationBusy || originalRequestNarrations.length > 0} onClick={() => void onOriginalRequestNarration()}><Headphones size={14} />{originalNarrationBusy ? 'Generating narration…' : originalRequestNarrations.length ? 'Narration ready' : 'Narrate request'}</button>}<button className="secondary-button" disabled={!canAnalyze || analysisBusy} onClick={() => void onAnalyze()}><Sparkles size={14} />{analysisBusy ? 'Reviewing…' : analyses.length ? 'Update recommendation' : 'Review request'}</button></div></div>
    ...
    <div className="audio-summary"><div><Headphones size={16} aria-hidden="true" /><strong>Source briefing</strong></div>{sourceBriefings.length ? sourceBriefings.map((artifact) => <audio key={artifact.id} controls preload="none" src={artifact.contentUrl} aria-label={`Play source briefing for ${request.title}`}>Audio could not be played.</audio>) : <span>{isRecruiterDemo ? 'Audio unavailable in this public demo.' : 'No audio attached.'}</span>}</div>
    <div className="summary-block"><div className="summary-label"><Workflow size={15} />Why this matters</div><p>{request.businessProblem}</p></div>
    <div className="original-request"><span>Original request</span><p>{request.requestText ?? request.businessProblem}</p>{originalRequestNarrations.map((artifact) => <audio className="audio-artifact" key={artifact.id} controls preload="none" src={artifact.contentUrl} aria-label={`Play original request narration for ${request.title}`}>Your browser cannot play this original request narration.</audio>)}{isRecruiterDemo && <span className="audio-note">Original request narration is unavailable in this public demo.</span>}</div>
    <div className="next-action-card" id="next-action"><span>Next step</span><strong>{nextAction}</strong>{needsClarification && <a href="#clarification-task">Answer required question <span aria-hidden="true">→</span></a>}{!needsClarification && !latest && <button className="primary-button" onClick={() => void onAnalyze()} disabled={analysisBusy}><Sparkles size={14} />Review request</button>}</div>
    <details className="evidence-disclosure request-details"><summary>Request details</summary><div className="field-grid"><DetailField label="Desired outcome" value={request.desiredOutcome} /><DetailField label="Requester role" value={request.requesterRole} /><DetailField label="Intended users" value={request.intendedUsers.join(', ')} /><DetailField label="Data sources" value={request.dataSources.join(', ')} /></div></details>
    ...
    {analyses.length > 0 && <details className="evidence-disclosure"><summary>Analysis history</summary><div className="analysis-history"><h3>Analysis history</h3>{analyses.map((run, index) => <div className="analysis-history-row" key={run.id}><span>Run {analyses.length - index}</span><strong>{run.outcome.replaceAll('_', ' ')}</strong><small>{run.provider} / {run.model} · prompt {run.promptVersion} · {run.latencyMs} ms · {new Date(run.createdAt).toLocaleString()}</small></div>)}</div></details>}
    <details className="evidence-disclosure"><summary>Audit trail</summary><div className="audit-block"><div className="audit-heading"><div><h3>Audit trail</h3><span>Requester, AI, system, automation, and human actions remain separate.</span></div><FileClock size={17} /></div><div className="timeline">{request.auditEvents.map((event) => <div className="timeline-item" key={event.id}><div className={`timeline-dot actor-${event.actorType}`}><FileClock size={10} /></div><div><strong>{event.actorName} · {event.actorType}</strong><p>{event.description}</p></div><time>{new Date(event.createdAt).toLocaleTimeString()}</time></div>)}</div></div></details>
  </>
}
...
function AutomationEvidence({ request, analyses, automations, artifacts, audioBusy, onRetry, onAudio }: { request: RequestDetail; analyses: AnalysisRun[]; automations: AutomationAttempt[]; artifacts: ArtifactRecord[]; audioBusy: boolean; onRetry: (attemptId: string) => Promise<void>; onAudio: () => Promise<void> }) {
  ...
  return <section className="automation-evidence">
    <div className="analysis-heading"><div><p className="eyebrow">Execution evidence</p><h3>Automations and optional audio</h3></div><span>{request.syntheticDemoSafe ? 'Audio available' : 'Audio unavailable'}</span></div>
    <p className="analysis-summary">{isRecruiterDemo ? 'The written recommendation is authoritative in this public demo. Live audio is unavailable here.' : 'The written recommendation is authoritative. Audio is generated only for explicitly demo-safe requests.'}</p>
    {latestSummary && <div className="authoritative-text"><strong>Source briefing text</strong><p>{latestSummary}</p>{isRecruiterDemo ? <span className="audio-note">Audio unavailable in this public demo.</span> : <button className="play-button" disabled={!request.syntheticDemoSafe || audioBusy} onClick={() => void onAudio()}><Headphones size={14} />{audioBusy ? 'Loading audio…' : 'Generate source briefing'}</button>}</div>}
    {artifacts.filter((artifact) => artifact.artifactType === 'audio_briefing').map((artifact) => <audio className="audio-artifact" key={artifact.id} controls preload="none" src={artifact.contentUrl}>Your browser cannot play this audio artifact.</audio>)}
    <div className="automation-list">{automations.length === 0 ? <p>No automation attempts recorded.</p> : automations.map((attempt) => <div key={attempt.id}><span className={`automation-status ${attempt.status}`}>{attempt.status}</span><strong>{attempt.automationName}</strong><small>workflow {attempt.workflowVersion} · attempt {attempt.attemptNumber} · correlation {attempt.correlationId.slice(0, 8)} · execution {attempt.externalExecutionId ?? 'none'}</small>{['failed', 'unavailable'].includes(attempt.status) && !['generate-audio-briefing', 'generate-original-request-narration'].includes(attempt.automationName) && <button onClick={() => void onRetry(attempt.id)}>Retry</button>}</div>)}</div>
  </section>
}
...
function ReviewerWorkspace({ request, analyses, decisions, onDecision }: { request: RequestDetail; analyses: AnalysisRun[]; decisions: DecisionRecord[]; onDecision: (submission: HumanDecision) => Promise<void> }) {
  ...
  return <section className="review-workspace">
    ...
    <p className="analysis-summary">// {isRecruiterDemo ? 'A decision is bound to the latest successful analysis and request version. This demo applies the same transition and governance rules locally.' : 'A decision is bound to the latest successful analysis and this request version. The server rechecks legal transitions and governance eligibility.'}</p>
...
function AnalysisView({ run, request, clarificationAnswers, onAnswer }: { run: AnalysisRun; request: RequestDetail; clarificationAnswers: ClarificationAnswerRecord[]; onAnswer: (submission: ClarificationAnswerSubmission) => Promise<void> }) {
  ...
  return <div className="analysis-view" id="analysis-start">
    <div className="analysis-heading"><div><p className="eyebrow">Recommendation</p><h3>{analysis.normalizedTitle}</h3></div><span className="technical-label">{run.provider} · schema {run.schemaVersion}</span></div>
    <p className="analysis-summary">{analysis.reviewerSummary}</p>
    <details className="evidence-disclosure recommendation-evidence"><summary>Recommendation evidence</summary><div className="recommendation-grid"><div><span>AI recommendation</span><strong>{run.modelRecommendation?.replaceAll('_', ' ')}</strong></div><div><span>Governance result</span><strong>{run.systemRecommendation?.replaceAll('_', ' ')}</strong></div></div><div className="evidence-grid"><EvidenceList title="What we know" items={analysis.facts.map((item) => `${item.value} · ${item.source}`)} /><EvidenceList title="Assumptions to verify" items={analysis.assumptions.map((item) => `${item.value} · unconfirmed`)} /><EvidenceList title="Missing information" items={analysis.unknowns} /></div><div className="rule-list"><h4>Governance checks</h4>{run.ruleEvaluation.map((rule) => <div key={rule.rule}><span className={`rule-result ${rule.result}`}>{rule.result.replace('_', ' ')}</span><p><strong>{rule.rule.replaceAll('_', ' ')}</strong>{rule.explanation}</p></div>)}</div></details>
    {analysis.clarificationQuestions.length > 0 && <div className="clarification-list" id="clarification-task"><h4>Required question</h4>{analysis.clarificationQuestions.map((question) => {
      const answer = clarificationAnswers.find((item) => item.questionId === question.id)
      return <div className="clarification-card" key={question.id}><div><span>{question.blocking ? 'Required before review' : 'Helpful question'}</span><strong>{question.question}</strong><p>{question.blocking ? 'Name the person or role responsible for validating generated summaries.' : question.reason}</p></div>{answer ? <div className="saved-answer" role="status"><strong>Answer saved by {answer.actorName}</strong><p>{answer.answer}</p></div> : <ClarificationForm questionId={question.id} onAnswer={onAnswer} />}</div>
    })}</div>}
  </div>
}
...
function ClarificationForm({ questionId, onAnswer }: { questionId: string; onAnswer: (submission: ClarificationAnswerSubmission) => Promise<void> }) {
  const [answer, setAnswer] = useState('')
  const [saving, setSaving] = useState(false)
  async function save() {
    if (answer.trim().length < 2) return
    setSaving(true)
    try { await onAnswer({ questionId, answer, actorType: 'human', actorName: 'Reviewer' }) }
    finally { setSaving(false) }
  }
  return <div className="clarification-form"><textarea aria-label={`Reviewer answer to ${questionId}`} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Write the reviewer answer" /><button disabled={saving || answer.trim().length < 2} onClick={() => void save()}>{saving ? 'Saving…' : 'Save answer'}</button></div>
}
...
function RequestForm({ onClose, onSubmitted }: { onClose: () => void; onSubmitted: (request: RequestDetail) => Promise<void> }) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const candidate = { ...form, requestText: form.requestText?.trim() || form.businessProblem, intendedUsers: users.split(',').map((item) => item.trim()).filter(Boolean), dataSources: sources.split(',').map((item) => item.trim()).filter(Boolean), currentProcess: form.currentProcess?.trim() || null }
  ...
  return <div className="modal-backdrop"><form className="new-request-modal intake-form" role="dialog" aria-modal="true" aria-labelledby="request-form-title" onSubmit={(event) => void handleSubmit(event)} noValidate><button type="button" className="modal-close" onClick={onClose} aria-label="Close intake form"><X size={18} /></button><p className="eyebrow">{isRecruiterDemo ? 'Local demo intake' : 'Persisted intake'}</p><h2 id="request-form-title">Submit a request</h2><p>{isRecruiterDemo ? 'Use fictional information only. The shared contract validates the request before it is stored in this browser.' : 'All fields are fictional demo information. The server validates the same shared contract before writing anything.'}</p>
